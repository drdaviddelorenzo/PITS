...
result_24 <- calculate_power(
  ...
  test = "level", n_sim = 1000, seed = 123
)
print(result_24)
```
...
grid <- build_param_grid(
  n_post       = c(18, 24, 30, 36, 42),
  level_change = level_change,
  sigma        = c(1.0, round(params$sigma, 1), 2.0, 2.5),
  rho          = c(0.2, 0.4, 0.6),
  n_pre        = params$n_pre,
  baseline     = params$baseline
)
grid_results <- run_power_grid(grid, n_sim = 500, seed = 42, verbose = FALSE)
```
...
result_multi <- calculate_power_multi(
  sites, test = "level", n_sim = 1000, seed = 123
)
print(result_multi)
```
